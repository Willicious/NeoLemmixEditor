...
namespace NLEditor
{
    internal static class ImagePreview
    {
        public static void PreviewPiece(string key, PictureBox pic, Style style)
        {
            ...
            int frameIndex = (ImageLibrary.GetObjType(key).In(C.OBJ.PICKUP, C.OBJ.EXIT_LOCKED, C.OBJ.BUTTON, C.OBJ.TRAPONCE)) ? 1 : 0;
            ...
            {
                Color blendColor = style?.GetColor(C.StyleColor.ONE_WAY_WALL) ?? C.NLColors[C.NLColor.OWWDefault];
